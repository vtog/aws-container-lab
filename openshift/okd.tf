...
resource "null_resource" "ansible" {
  provisioner "local-exec" {
    ...
    command     = "aws ec2 wait instance-status-ok --region ${var.aws_region} --instance-ids ${aws_instance.okd-master1.id} ${aws_instance.okd-node1.id} ${aws_instance.okd-node2.id} --profile vtog && ansible-playbook ./playbooks/deploy-okd.yaml"
  }
}
