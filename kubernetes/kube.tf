...
resource "null_resource" "ansible" {
  provisioner "local-exec" {
    ...
    command     = "aws ec2 wait instance-status-ok --region ${var.aws_region} --profil ${var.aws_profile} --instance-ids ${aws_instance.kube-master1.id} ${aws_instance.kube-node1.id} ${aws_instance.kube-node2.id} --profile vtog && ansible-playbook ./playbooks/deploy-kube.yaml"
  }
}
